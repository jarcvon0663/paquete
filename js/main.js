...
function cargarProductos(productosElegidos) {
    const contenedorProductos = document.getElementById("contenedor-productos"); // Asegúrate de que este es el ID correcto
    ...
    productosElegidos.forEach(producto => {
        ...
        div.innerHTML = `
            ...
            <div class="producto-detalles">
                ...
                <button class="producto-agregar" data-id="${producto.id}" target="_blank">Descargar</button>
            </div>
        `;
    ...
    const downloadButtons = document.querySelectorAll(".producto-agregar");
    downloadButtons.forEach(button => {
        button.addEventListener("click", function() {
            const downloadLink = document.createElement("a");
            downloadLink.href = this.getAttribute("data-id");
...
function agregarAlCarrito(e) {
    ...
    if (productoExistente) {
        ...
        Toastify({
            text: "pronto se abrira la página de descarga",
            duration: 3000,
            close: true,
            gravity: "top", // `top` or `bottom`
            position: "right", // `left`, `center` or `right`
